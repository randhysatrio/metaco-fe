    ...
    <div className="w-full h-96 relative">
      ...
      <Swiper navigation={{ prevEl: '.prev', nextEl: '.next' }} modules={[Navigation]} loop={true} className="h-full w-full">
        {banners.map((banner, index) => (
          <SwiperSlide key={index} className="h-full w-full flex items-center justify-center relative">
            <img src={banner} alt={`banner_${index + 1}`} className="h-full object-cover" />
